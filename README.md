# 학습 보관함 (구 "5분 학습", 저장소명: 5minuate_study)
...
- AI가 학습 요약과 퀴즈 15문제를 자동 생성 (약 10~20초, 대기 중엔 학습 팁이 순환 노출됨)
...
- 완료 후에도 **다음 학습 계속하기**로 바로 다음 날 분량을 이어갈 수 있음 (스트릭은 실제 날짜 기준으로 하루 1회만 증가, 하루를 걸러도 목표당 1회는 스트릭 프리즈로 보호됨)
- 오답은 자동으로 오답노트에 쌓이고, 간격 반복(라이트너 상자) 방식으로 다음 테스트에 우선 출제됨
...
- **내 문제집 복습**: 담아둔 문제도 정답을 바로 보여주지 않고 직접 다시 풀어보는 방식으로 복습
- **복습 믹스**: 진행 중인 목표가 2개 이상이면 홈에 노출 — 여러 목표의 문제를 섞어 빠르게 훑어봄(스트릭에는 영향 없음)
...
### 6. 친구 & 학습 공유
- 친구 화면에서 닉네임으로 사용자를 검색해 친구 추가/수락
- 내 목표·세션·문제집을 친구에게 앱 내로 바로 보내거나, 외부(카카오톡/문자 등)로 공유 링크를 복사해 전달 가능 — 문제는 AI 재생성 없이 그대로 전달됨
- 받은 공유는 확인 후 **내 문제집에 저장**(북마크)하거나, 문제가 포함된 공유라면 기한만 입력해 바로 **학습 목표로 가져오기**도 가능(난이도는 공유자 목표에서 자동 상속)
- 친구 간 학습 점수 리더보드 제공

### 7. 업적
- 첫 세션 완료, 7일/30일 연속 달성, 만점 5회, 오답풀 완전 정리 등 배지 5종 — 설정 하단 "업적" 메뉴에서 확인

### 8. 설정
- 계정 로그아웃 / 지금 동기화
- 학습 데이터(오답 목록·내 문제집·녹음 모음) 바로가기
- 배경 스킨(라이트/차콜/네이비 등) 변경
...
- **선생님**: 설정에서 학원 초대 코드로 등록 → 반 생성(자동 발급되는 참여 코드) → 숙제 출제(AI로 문제 후보 생성 후 선택, 또는 직접 작성) → 반 상세 화면에서 학생별 완료 여부·점수·오답 문항(학생이 실제로 고른 답 vs 정답)·미제출/마감 임박 배지까지 확인, 코멘트 남기기, 잘못 낸 숙제는 삭제 가능
- **학생**: 반 참여 코드 입력 → 홈 화면 배너/🎓 아이콘으로 오늘의 숙제 확인 → 풀이·제출(채점은 클라이언트가 아닌 서버 RPC가 실제 정답과 대조해 수행) → 이미 제출한 숙제는 재풀이 대신 내 답/정답과 선생님 코멘트를 복습하는 화면으로 전환
...
이 프로젝트가 어떤 방향으로, 왜 이렇게 만들어졌는지는 `docs/` 아래 기획 문서를 참고하세요.

- [`docs/ceo_prompt.md`](docs/ceo_prompt.md) — 제품 비전·제약·성공 기준(CEO 프롬프트)
- [`docs/planning_document.md`](docs/planning_document.md) — 페르소나·유저 저니·UX 원칙·성공 지표
- [`docs/feature_definition.md`](docs/feature_definition.md) — 기능별 F-ID 상세 명세(F-01~)
- [`docs/architecture_decision.md`](docs/architecture_decision.md) — 기술 스택·데이터 모델 결정
...
배포 URL·Cloudflare Worker 이름(`5minuatestudy`)·저장소 이름은 표시 이름 리브랜딩("학습 보관함")과 무관하게 그대로 유지합니다 — 이미 설치된 PWA 사용자의 접속이 끊기지 않도록 하기 위함입니다.
...
- `002~007` — 학원용 선생님/학생 기본 기능: `profiles`(역할) · `academies`/`academy_members`(학원, 1개만 시드) · `classes`/`class_members`(반, 초대코드) · `assignments`/`assignment_questions`/`assignment_submissions`(숙제·채점) · `submit_assignment` RPC(서버 측 채점, 오답 문항·학생 답 기록) · RLS 인가 강화 및 재귀 정책 수정
- `008~015` — 학원 기능 확장: 수업자료(첨부파일 포함) · 역할 전환(교사↔학생) · 숙제 대상 지정(특정 학생만) · 닉네임 · 학원 공지 · 문제 신고 · 숙제 코멘트
- `016~019` — 친구·학습 공유: `friendships`(친구 요청/수락) · `study_shares`(앱 내 학습 공유함) · `search_user_profiles`/`accept_friend_request` RPC(RLS 우회용 security definer, 친구 검색·수락 버그 수정 포함)
